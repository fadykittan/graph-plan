...
def independentPair(a1, a2):
  ...
  # check if a1 and a2 inconsistent
  isInconsistent = False
  for effect in a1.getAdd():
    if a2.isNegEffect(effect):
      isInconsistent = True
      break

  for effect in a2.getAdd():
    if a1.isNegEffect(effect):
      isInconsistent = True
      break

  # check if a1 and a2 interfere one with the other
  isInterfere = False
  ...
  for pre in a2.getPre():
    if a1.isNegEffect(pre):
      isInterfere = True
      break

  return not isInconsistent and not isInterfere
